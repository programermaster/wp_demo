...
function ch4_br_create_book_post_type() {
    ...
    register_post_type( 'book_reviews',
        array(
            ...
            //start part of slug for book /../awesome_book_reviews/book1/
...
function ch4_br_add_book_type_item() {
    ...
    $submenu['edit.php?post_type=book_reviews'][501] =
        array( 'Book Type', 'manage_options', admin_url( '/edit-tags.php?taxonomy=book_reviews_book_type&post_type=book_reviews' ) );
}
...
function ch4_br_column_ordering( $vars ) {
    ...
    if ( isset( $vars['orderby'] ) &&  'book_reviews_author' == $vars['orderby'] ) {
        $vars = array_merge( $vars, array( 'meta_key' => 'book_author', 'orderby' => 'meta_value' ) );
    } elseif ( isset( $vars['orderby'] ) && 'book_reviews_rating' == $vars['orderby'] ) {
        $vars = array_merge( $vars, array( 'meta_key' => 'book_rating', 'orderby' => 'meta_value_num' ) );
    }
    return $vars;
}
...
// otherwise serve the file from the plugin
            //die(locate_template( array( 'single-book_reviews.php' ) ));
...
// Preparation of query array to retrieve 5 book reviews
    $query_params = array( 'post_type' => 'book_reviews', 'post_status' => 'publish', 'posts_per_page' => 2 );
