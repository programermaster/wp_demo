...
// using function parameter
    $creation_query = 'CREATE TABLE IF NOT EXISTS ' . $prefix . 'ch7_bug_data (
...
function ch7bt_config_page() {
    ...
    <h2>Bug Tracker <a class="add-new-h2" href="<?php echo add_query_arg( array( 'page' => 'ch7bt-bug-tracker', 'id' => 'new' ), admin_url('options-general.php') ); ?>">
            Add New Bug</a>
    </h2>
                ...
                <td>
                    <select name="bug_status">
                        ...
                        foreach( $bug_statuses as $status_id => $status ) {
                            // Add selected tag when entry matches  existing bug status
...
// Check if user has proper security level
    if ( !current_user_can( 'manage_options' ) )
        wp_die( 'Not allowed' );// Check if nonce field is present for security
...
// Check that user has proper security level
    if ( !current_user_can( 'manage_options' ) )
        wp_die( 'Not allowed' );// Check if nonce field is present
...
function ch7bt_shortcode_list() {
    ...
    $output = '<form action="' .  basename(get_permalink()) . '" method="get" id="ch7_bt_search">';
...
// Check if any bugs were found
    if ( !empty( $bug_items ) ) {
        ...
        $output .= '<th style="width: 300px">Title</th>';
        $output .= '<th>Version</th>
                    <th style="width: 300px">Desciption</th></tr>';
